flectra.define('smt_beton.dashboard', function(require) {
    "use strict";
    ...
    var MarketingDashboardRenderer = KanbanRenderer.extend({
        ...
        _render: function() {
            ...
            var self = this;
            self._rpc({
                route: '/smt_beton/smt_beton',
                params: {},}).done(function
            (result) {
                if (result) {
                    $('#total_projects').html("<h2>" + "result" + "</h2>");
                    $('#total_sprints').html("<h2>" + "Hasil2" + "</h2>");
                    $('#total_stories').html("<h2>" + "Hasil3" + "</h2>");
                    $('#total_tasks').html("<h2>" + "Hasil4" + "</h2>");
                }
            });
    ...
    var MyMarketingDashboard = KanbanView.extend({
        config: _.extend({}, KanbanView.prototype.config, {
            Model: MarketingDashboardModel,
            Renderer: MarketingDashboardRenderer,
            Controller: MarketingDashboardController,
        }),
        display_name: _lt('Marketing Dashboard'),
        icon: 'fa-dashboard',
        searchview_hidden: true,
    });

    view_registry.add('marketing_dashboard', MyMarketingDashboard);
