...
class MrpRecord(models.TransientModel):
    _name = 'mrp.record'
    ...
    def _compute_duration(self):
        ...
        if not workorder.workcenter_id.record_real_time:
    ...
    def onchange_workorder_id(self):
        if self.workorder_id and self.workorder_id.workcenter_id.partial_record:
    ...
    def on_barcode_scanned(self, barcode):
            ...
            if nomenclature:
                ...
                if scann['type'] == 'mrp_operation':
                    ...
                    if scann['code'] == self.workorder_id.code:
                        if self.workorder_id.workcenter_id.partial_record:
                            self.qty_producing += 1
                            self.info_message = _('Incremented quantity')
                            return
                    """
        ...
        if self.production_id and self.workorder_id and self.worker_id:
            if production != self.production_id or self.workorder_id != workorder or self.worker_id != worker or barcode == '#save':
                self.success_message = _('Production recorded for operation %s') % self.workorder_id.name
                if self.qty_producing > 0:
                    self.record()  # workorder=self.workorder_id, worker=self.worker_id, qty_producing=self.qty_producing)
        ...
        if self.production_id and self.workorder_id and self.worker_id and self.qty_producing > 0:
            self.info_message = _('System is ready for record production %s operation %s with %s') % (
                production.name, workorder.name, worker.name)

        return

    @api.model
    def record(self):  # , workorder, worker, qty_producing):
    ...
    def default_get(self, fields):
        defaults = super(MrpRecord, self).default_get(fields)
    ...
    def do_record(self):
        next = True
        if self.production_id and self.workorder_id and self.worker_id:
            next = self.record()  # workorder=self.workorder_id, worker=self.worker_id, qty_producing=self.qty_producing)
        action = self.env.ref('deltatech_mrp_record.action_mrp_record').read()[0]
